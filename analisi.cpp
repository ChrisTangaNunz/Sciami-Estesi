...
void plot_histogram(const char* filePath, const char* output_file) {
...
void plot_RatevsParameter(const char* filePath, const char* outputName, const char* parameterName, const char* channel) {
    ...
    TCanvas *canvas = new TCanvas(outputName, Form("Rates vs %s", parameterName), 800, 600);
    canvas->SetGrid();
...
int analisi(){
    ...
    string files[] = {
        ...
        "clean_data/tempi_corretti_canale_3_231123.txt"
    };
